...
public class EditDistance {
  ...
  public List<EditOperation> calculatePath() {
    ...
    while (i > 0 && j > 0 && dist > 0) {
      ...
      if (dist == ins + 1) {
        addOrUpdate(ops, EditOperation.Kind.INSERT, 1);
        j--;
      } else if (dist == del + 1) {
        addOrUpdate(ops, EditOperation.Kind.DELETE, 1);
        i--;
      } else {
        if (dist == sub) addOrUpdate(ops, EditOperation.Kind.SKIP, 1);
        else addOrUpdate(ops, EditOperation.Kind.SUBSTITUTE, 1);
        i--; j--;
      }
    ...
    if (i == 0) addOrUpdate(ops, EditOperation.Kind.INSERT, j);
    else if (j == 0) addOrUpdate(ops, EditOperation.Kind.DELETE, i);
    else addOrUpdate(ops, EditOperation.Kind.SKIP, i);
